...
private int cariPassengerIDTerbaru() {
    List<model.Passenger> list = passengerController.getAllPassengers();
    int maxID = 0;
    for (model.Passenger p : list) {
...
private int cariTicketIDByCode(String ticketCode) {
    List<model.Ticket> list = ticketController.getAllTickets();
    for (model.Ticket t : list) {
...
private void prosesRingkasan() {
    // 1. Simpan data penumpang ke database
    String resultPassenger = passengerController.addPassenger(
        passengerName, passengerEmail, passengerPhone, passengerNationality, passengerType
    );

    if (!resultPassenger.equals("OK")) {
        JOptionPane.showMessageDialog(this, resultPassenger, "Gagal Menyimpan Data Penumpang", JOptionPane.ERROR_MESSAGE);
        this.dispose();
        return;
    }

    // Ambil ID passenger yang baru saja dibuat (cari dari daftar, karena addPassenger tidak return ID)
    newPassengerID = cariPassengerIDTerbaru();

    // 2. Ambil data jadwal untuk ditampilkan
    ...
    // 4. Buat tiket dengan status Pending
    String resultTicket = ticketController.bookTicket(newPassengerID, selectedScheduleID, tripType, passengerType);

    if (!resultTicket.startsWith("OK")) {
        JOptionPane.showMessageDialog(this, resultTicket, "Gagal Membuat Tiket", JOptionPane.ERROR_MESSAGE);
        this.dispose();
        return;
    }

    // Ambil kode tiket dari hasil "OK|TKT-xxxx"
    newTicketCode = resultTicket.split("\\|")[1];
    newTicketID = cariTicketIDByCode(newTicketCode);

    // 5. Tampilkan ke label-label
