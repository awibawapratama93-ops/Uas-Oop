...
private void setupSearchPlaceholder() {
    ...
    private void initComponents() {
        ...
        lblPageInfo.setForeground(new java.awt.Color(10, 61, 89));
...
private void applySelectedRow(int row) {
    ...
    if (row == -1) {
        int lastRow = tblSchedule.getRowCount() - 1;
        if (lastRow >= 0) {
            tblSchedule.setRowSelectionInterval(lastRow, lastRow);
            applySelectedRow(lastRow);
        }
    } else if (row > 0) {
        int newRow = row - 1;
        tblSchedule.setRowSelectionInterval(newRow, newRow);
...
private void refreshTable() {
    // 1. Ambil semua data (hasil join Boat+Route dari controller)
    List<Schedule> allSchedules = scheduleController.getAllSchedules();

    // 2. Filter berdasarkan pencarian (nama kapal atau rute)
    ...
    if (keyword.isEmpty()) {
        ...
        for (Schedule s : allSchedules) {
            ...
            if (boatName.toLowerCase().contains(keyword.toLowerCase())
                    || routeName.toLowerCase().contains(keyword.toLowerCase())) {
    ...
    int totalRows = filteredSchedules.size();
    ...
    // 4. scheduleList cuma berisi yang tampil di halaman ini
    //    (tblScheduleMouseClicked bergantung pada ini, supaya klik baris tetap akurat)
    scheduleList = new ArrayList<>(filteredSchedules.subList(startIndex, endIndex));
