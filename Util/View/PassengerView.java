...
private void applySelectedRow(int row) {
    ...
    if (row == -1) {
        int lastRow = tblPassenger.getRowCount() - 1;
        if (lastRow >= 0) {
            tblPassenger.setRowSelectionInterval(lastRow, lastRow);
            applySelectedRow(lastRow);
        }
    } else if (row > 0) {
        int newRow = row - 1;
        tblPassenger.setRowSelectionInterval(newRow, newRow);
    ...
    if (row == -1) {
        if (lastRow >= 0) {
            tblPassenger.setRowSelectionInterval(0, 0);
            applySelectedRow(0);
        }
    } else if (row < lastRow) {
        int newRow = row + 1;
